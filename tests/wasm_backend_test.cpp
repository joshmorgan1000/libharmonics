...
TEST(WasmBackendTest, DeviceRoundTripPreservesTensor) {
    ...
    auto dev = harmonics::wasm_to_device(t);
    auto host = harmonics::wasm_to_host(dev);
...
TEST(WasmBackendTest, SelectWasmBackendMatchesBuild) {
#if HARMONICS_HAS_WASM
    EXPECT_NE(harmonics::select_wasm_backend(), harmonics::WasmBackend::None);
#else
    EXPECT_EQ(harmonics::select_wasm_backend(), harmonics::WasmBackend::None);
#endif
}
