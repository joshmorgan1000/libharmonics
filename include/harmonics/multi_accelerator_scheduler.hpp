...
/**
 * Scheduler that coordinates heterogeneous runtimes for partitioned graphs.
 *
 * Each graph partition runs in its own CycleRuntime configured with the
 * backend specified in the deployment descriptor. Boundary tensors are
 * forwarded through in-memory buses, allowing CPU, GPU and FPGA runtimes
 * to cooperate within a single process.
 */
class MultiAcceleratorScheduler {
  public:
    /// Construct the scheduler with already partitioned graphs.
    explicit MultiAcceleratorScheduler(std::vector<HarmonicGraph> parts,
